...
function formatPhoneEntry(data) {
  ...
  if (data.status === 'success') {
    ...
    entry += `├─ 👤 Họ tên: [Không xác định]\n`;
    if (data.uid) {
      entry += `├─ 🆔 UID: ${data.uid}\n`;
    }
    entry += `├─ 🖼️  Avatar: N/A\n`;
    entry += `├─ 📊 Trạng thái: ${statusDetail}\n`;
    entry += `├─ 💬 Nội dung: [Không gửi được]\n`;
 ...
 */
export function logSendMessageFailed(phone, error, uid = null) {
  try {
    ensureLogDir();
    phoneCounter++;
    const entry = formatPhoneEntry({
